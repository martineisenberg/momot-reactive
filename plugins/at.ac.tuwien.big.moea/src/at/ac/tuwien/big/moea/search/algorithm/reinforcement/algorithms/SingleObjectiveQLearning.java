...
public class SingleObjectiveQLearning<S extends Solution> extends AbstractSOTabularRLAgent<S> {
   ...
   protected void iterate() {
      ...
      if(doneStatus == DoneStatus.MAX_LENGTH_REACHED || doneStatus == DoneStatus.FINAL_STATE_REACHED) {
         ...
         if(this.recordInterval > 0) {
            ...
            if(terminateAfterEpisodes > 0 && terminateAfterEpisodes <= epochCount) {
            ...
            if(savePath != null && epochCount > 0 && epochCount % this.recordInterval == 0) {
               ...
               printIfVerboseMode("Saving rewards at epoch " + epochCount + " after "
                     + (System.currentTimeMillis() - startTime) / 1000.0 + " seconds");

               saveRewards(savePath, framesList, rewardEarned, timePassedList, meanRewardEarned);
            }
         }
   ...
   private void updateQ(final List<ApplicationState> state, final List<ApplicationState> action,
      ...
      //
      // System.out.println(nextState.hashCode());
